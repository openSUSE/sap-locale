...
/* add user defined area - linux@sap.com */
#include <sjisudc.c>
...
#define BODY \
  {									      \
    ...
    else if (__builtin_expect (ch > 0xea && ch < 0xf0, 0) 		      \
	...
	if (__builtin_expect (ch2 < 0x40, 0))				      \
	  {								      \
	    ...
	    STANDARD_FROM_LOOP_ERR_HANDLER (1);				      \
		 ...
		 || (__builtin_expect (idx > 0xeaa4 && idx < 0xf040, 0))      \
		 || __builtin_expect (idx > 0xfcfc, 0))			      \
	  {								      \
	    /* This is illegal.  */					      \
	  ...
	  {								      \
	    ...
	    else if (idx >= 0xf040 && idx <= 0xfcfc)			      \
	      ch = to_ucs4_udc[idx - 0xf040];				      \
	    else							      \
  ...
  {									      \
	...
	if (ch >= 0x0391 && ch <= 0x0451)				      \
	  ...
	  cp = from_ucs4_cjk[ch - 0x02010];				      \
        else if (ch >= 0xe000 && ch <= 0xe9fb)                                \
          cp = from_ucs4_udc[ch - 0x0e000];                                   \
